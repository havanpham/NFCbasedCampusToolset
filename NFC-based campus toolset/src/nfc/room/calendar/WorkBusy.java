...
public class WorkBusy extends Activity implements OnClickListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		// btnDelete.setCompoundDrawablesWithIntrinsicBounds(0, 0,
		// R.drawable.delete_icon, 0);
		// btnDelete.setOnClickListener(this);

		Button btnOK = (Button) findViewById(R.id.btnOK);
		btnOK.setCompoundDrawablesWithIntrinsicBounds(0, 0, R.drawable.ok_icon,
				0);
		...
		if (extras != null) {
			...
			if ((Integer.parseInt(hour1[0]) <= noon)) {
				...
				// hour[0] = hour[0] +" PM";
				// Toast.makeText(Calendar.this, hour[0],
		...
		arrayAdapter = new ListWorkAdapter(this, R.layout.work_busy_item, array) {
			 ...
			 * (non-Javadoc)
			 * 
			 * @see nfc.customview.ListWorkAdapter#getView(int,
		...
		list.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View arg1,
				...
				final RadioButton btn_edit = (RadioButton) dialog
						.findViewById(R.id.rbtn_edit);
				final RadioButton btn_delete = (RadioButton) dialog
						.findViewById(R.id.rbtn_delete);
		...
		while (mCursor.moveToNext()) {
			if ((valueExtras1.toString().compareTo(
					tf.format(mCursor.getLong(2)).toString()) <= 0)
					&& (valueExtras2.toString().compareTo(tf.format(
							mCursor.getLong(2)).toString())) >= 0) {
				arrayAdapter.add(new ListItem(R.drawable.yammi_icon, mCursor
						.getString(1), mCursor.getString(0) + "-"
						+ df.format(mCursor.getLong(2)), mCursor.getLong(3),
